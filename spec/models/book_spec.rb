...
RSpec.describe Book, type: :model do
  ...
  describe 'instance variables' do
    ...
    it '.sort_average_rating' do
      expect(Book.sort_average_rating('ASC')).to eq([@hp_sorcerer, @hp_prisoner]) 
      expect(Book.sort_average_rating('DESC')).to eq([@hp_prisoner, @hp_sorcerer]) 
    end

    it '.sort_number_of_pages' do
      expect(Book.sort_number_pages('ASC')).to eq([@hp_sorcerer, @hp_prisoner])
      expect(Book.sort_number_pages('DESC')).to eq([@hp_prisoner, @hp_sorcerer])
    end

    it '.sort_number_of_reviews' do
      expect(Book.sort_number_reviews('ASC')).to eq([@hp_sorcerer, @hp_prisoner])
      expect(Book.sort_number_reviews('DESC')).to eq([@hp_prisoner, @hp_sorcerer])
    end
  end
